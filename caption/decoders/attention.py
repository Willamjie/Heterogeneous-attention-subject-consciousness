...
class AttnDecoder(caption.decoders.vanilla.Decoder):
  ...
  def gen_memory_key_value(self, enc_memories):
    ...
    sentinel= torch.sigmoid(self.attn_linear_context(enc_memories))
    return memory_keys, memory_values,sentinel
  ...
  def forward(self, inputs, enc_states, enc_memories, enc_masks, return_attn=False):
    ...
    '''
    memory_keys, memory_values ,sentinel= self.gen_memory_key_value(enc_memories)
    ...
    for t in range(inputs.size(1)):
      ...
      attn_score1, attn_memory1 = self.attn1(outs,
                                          memory_keys, memory_values, enc_masks,outs)
      p=round(random.uniform(0.1, 0.5), 1)
      attn_score=attn_score*p+attn_score1*(1-p)
      step_attns.append(attn_score)
      attn_memory=attn_memory*p+attn_memory1*(1-p)
...
class BUTDAttnDecoder(AttnDecoder):
  ...
  def __init__(self, config):
    ...
    self.lang_lstm = nn.LSTMCell(
      ...
      self.config.hidden_size, bias=True)
    # attentions
    self.attn = GlobalAttention(self.config.hidden_size, self.config.attn_size,'local')#self.config.attn_type
    if self.config.attn_type == 'mlp':
  ...
  def init_dec_state(self, batch_size):
    ...
    for i in range(2): # (hidden, cell)
      states.append(torch.zeros((2, batch_size, self.config.hidden_size),dtype=torch.float32).to(param.device))
    return states
