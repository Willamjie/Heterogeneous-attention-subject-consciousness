...
class MemoryFlowDecoder(ContentFlowAttentionDecoder):
  def __init__(self, config):
    ...
    self.sentinal_layer = nn.Sequential(
      ...
      nn.ReLU(),
      nn.Linear(self.config.hidden_size, 1))
    self.linear=nn.Linear(self.config.hidden_size,10,bias=True)
  ...
  def forward(self, inputs, enc_globals, enc_memories, enc_masks, flow_edges, return_attn=False):
    ...
    for t in range(inputs.size(1)):
      ...
      memory_keys, memory_values,_ = self.gen_memory_key_value(enc_memories)
      ...
      content_attn_score, content_attn_memory = self.attn(h_attn_lstm,
        memory_keys, memory_values, enc_masks)
      # content_attn_score1, content_attn_memory1 = self.attn1(h_attn_lstm,
      #                                                        memory_keys, memory_values, enc_masks,h_attn_lstm)
      # #print(content_attn_score1.shape, content_attn_memory1.shape)
      # p = round(random.uniform(0.1, 0.5), 1)
      # content_attn_score, content_attn_memory = content_attn_score * p + content_attn_score1 * (
      #           1 - p), content_attn_memory * p + content_attn_memory1 * (1 - p)
      ...
      flow_attn_score = torch.stack([x.view(batch_size, max_attn_len) \
        for x in [flow_attn_score_1,flow_attn_score_2,flow_attn_score_3]], 2)
      ...
      # write: update memory keys and values
      # print(h_lang_lstm.shape,h_lang_lstm.unsqueeze(1).expand(-1, max_attn_len, -1).shape, enc_memories.shape)
      # individual_vectors = torch.cat([h_lang_lstm.unsqueeze(1).expand(-1, max_attn_len, -1), enc_memories], 2)#batch, enc_seq_len, attn_input_size
      # update_vectors = self.memory_update_layer(individual_vectors)
      # memory_size = update_vectors.size(-1) // 2
      #
      # erase_gates = torch.sigmoid(update_vectors[:, :, :memory_size])
      ...
      update_vectors = torch.mul(h_lang_lstm.unsqueeze(1).expand(-1, max_attn_len, -1), enc_memories)
      erase_gates=torch.sigmoid(update_vectors)
      add_vectors_score=torch.softmax(update_vectors,2)
      add_vectors=add_vectors_score*enc_memories
      # some words do not need to attend on visual nodes
      #h_lang_lstm(128,512)
      # sentinal=self.sentinal_layer(h_lang_lstm)
      # sentinal_gates = torch.sigmoid(sentinal)
      # #attn_score(128,10),sentinal_gates(128,1)
      # memory_attn_score = attn_score * sentinal_gates

      sentinal = self.linear(h_lang_lstm)
      memory_attn_score=torch.sigmoid(sentinal*attn_score)
