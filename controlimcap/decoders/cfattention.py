...
class ContentFlowAttentionDecoder(caption.decoders.attention.BUTDAttnDecoder):
  def __init__(self, config):
    ...
    self.attn1 = AdaptiveAttention(self.config.hidden_size, self.config.attn_size)
  ...
  def forward(self, inputs, enc_globals, enc_memories, enc_masks, flow_edges, return_attn=False):
    ...
    for t in range(inputs.size(1)):
      ...
      content_attn_score, content_attn_memory=content_attn_score*p+content_attn_score1*(1-p), content_attn_memory*p+content_attn_memory1*(1-p)
