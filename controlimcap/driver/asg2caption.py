  ...
  if opts.is_train:
    ...
    for measure_name, model_file in model_files.items():
      ...
      if not os.path.exists(tst_pred_file):
        _model.test(tst_reader, tst_pred_file, tst_model_file=model_file,outcap_format=opts.outcap_format)
      if not opts.no_evaluate:
        scores = evaluate_caption(
          None, tst_pred_file, ref_caps=ref_captions,outcap_format=opts.outcap_format)
